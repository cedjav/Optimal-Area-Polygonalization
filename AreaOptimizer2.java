...
public class AreaOptimizer2 {
	...
	public static void main(String[] args) {
		...
		GridPoint_2[] points=PointCloud_IO2.read(inputFile);
		if(args.length==2)
		{
			int[] drawingBounds=PointCloud_IO2.getBoundingBox(points); // get the bounding box [0..xmax]x[0..ymax]
			...
			float areaMin=op.computeArea(minimal); // compute the area of the polygon (this function MUST BE IMPLEMENTED)
			if(isValidMin==false)
			...
			float areaMax=op.computeArea(maximal); 
			if(isValidMax==false)
				System.out.println("The maximal polygon is not valid (or not defined)");
